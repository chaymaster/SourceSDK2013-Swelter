...
// When the engine is running and the drone is operating under power
// we don't let gravity affect him.
#define DRONE_GRAVITY 0.000

#define DRONE_GIB_COUNT			5 
#define DRONE_INGORE_WATER_DIST	384

// Sound stuff
#define DRONE_PITCH_DIST1		512
#define DRONE_MIN_PITCH1		(100)
#define DRONE_MAX_PITCH1		(160)
#define DRONE_WATER_PITCH1	(85)
#define DRONE_VOLUME1			0.55

#define DRONE_PITCH_DIST2		400
#define DRONE_MIN_PITCH2		(85)
#define DRONE_MAX_PITCH2		(190)
#define DRONE_WATER_PITCH2	(90)

#define DRONE_NOISEMOD_HIDE 5000

#define DRONE_BODYGROUP_BLADE	1
#define DRONE_BODYGROUP_BLUR	2
#define DRONE_BODYGROUP_OFF	0
#define DRONE_BODYGROUP_ON	1

// ANIMATION EVENTS
#define DRONE_AE_START_ENGINE			50
#define DRONE_AE_DONE_UNPACKING		51
#define DRONE_AE_OPEN_BLADE			52

//#define DRONE_GLOW_SPRITE	"sprites/laserdot.vmt"
#define DRONE_GLOW_SPRITE	"sprites/drone_eye.vmt"
...
// Private activities.
//-----------------------------------------------------------------------------
Activity ACT_DRONE_UNPACK;

//-----------------------------------------------------------------------------
// Drone Conditions
//-----------------------------------------------------------------------------
enum DroneConditions
{
	COND_DRONE_START_ATTACK = LAST_SHARED_CONDITION,	// We are able to do a bombing run on the current enemy.
};
...
enum DroneSchedules
{
	SCHED_DRONE_ATTACK_HOVER = LAST_SHARED_SCHEDULE,
	SCHED_DRONE_DEPLOY,
	SCHED_DRONE_REGROUP,
...
enum DroneTasks
{
	TASK_DRONE_HOVER = LAST_SHARED_TASK,
	TASK_DRONE_UNPACK,
	TASK_DRONE_FIND_SQUAD_CENTER,
	TASK_DRONE_FIND_SQUAD_MEMBER,
...
//-----------------------------------------------------------------------------
// Purpose: Turns the drone into a physics corpse when dying.
//-----------------------------------------------------------------------------
void CNPC_Drone::Event_Dying(void)
{
	DestroySmokeTrail();
	SetHullSizeNormal();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Drone::GatherConditions()
{
...
void CNPC_Drone::PrescheduleThink(void)
{
	...
	if (m_flWaterSuspendTime > gpGlobals->curtime)
	{
		...
		// Reduce engine power so that the drone lifts out of the water slowly.
...
void CNPC_Drone::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	...
	if (pPlayer)
	{
		...
		// Don't take damage from NPCs or server ragdolls killed by the drone
...
//-----------------------------------------------------------------------------
// Purpose: Drone is out of control! (dying) Just explode as soon as you touch anything!
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
int	CNPC_Drone::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	else if (info.GetDamageType() & DMG_CLUB)
	{
		...
		//		 then make that adjustment. This is a simple heuristic. The drone will be
...
//-----------------------------------------------------------------------------
// Purpose: Explode the drone if it's damaged while crashing
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
void CNPC_Drone::HandleAnimEvent(animevent_t *pEvent)
{
	...
	case DRONE_AE_DONE_UNPACKING:
		m_flNextBurstTime = gpGlobals->curtime + 2.0;
		break;

	case DRONE_AE_OPEN_BLADE:
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Drone::MoveToTarget(float flInterval, const Vector &vMoveTarget)
{
	if (flInterval <= 0)
	{
		return;
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Drone::MoveCollisionMask(void)
{
...
void CNPC_Drone::ComputeSliceBounceVelocity(CBaseEntity *pHitEntity, trace_t &tr)
{
	if (pHitEntity->IsAlive() && FClassnameIs(pHitEntity, "func_breakable_surf"))
	{
		...
		// something that's alive. Anyway, prevent the drone bouncing off the pane of glass,
		// since this impact will shatter the glass and let the drone through.
		return;
	}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Drone::CheckCollisions(float flInterval)
{
...
void CNPC_Drone::PlayFlySound(void)
{
	float flEnemyDist;
	...
	if (m_flWaterSuspendTime > gpGlobals->curtime)
	{
		...
		if (m_bDirtyPitch)
		{
			...
			m_nEnginePitch2 = DRONE_WATER_PITCH2;
	...
	else if (GetEnemy() && IsAlive())
	{
		...
		else if (m_bDirtyPitch)
		{
			...
			m_flEnginePitch1Time = gpGlobals->curtime + 0.1f;
			m_nEnginePitch2 = DRONE_MIN_PITCH2;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Drone::MoveExecute_Alive(float flInterval)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Drone::SpinBlades(float flInterval)
{
	if (!m_bBladesActive)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Drone::StopLoopingSounds(void)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Drone::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
float CNPC_Drone::GetDefaultNavGoalTolerance()
{
	return GetHullWidth();
}
...
//-----------------------------------------------------------------------------
// Drones that have been hacked by Alyx get more engine power (fly faster)
//-----------------------------------------------------------------------------
