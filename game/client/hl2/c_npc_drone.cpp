...
class C_NPC_Drone : public C_AI_BaseNPC
{
public:
	C_NPC_Drone() {}

	DECLARE_CLASS( C_NPC_Drone, C_AI_BaseNPC );
	DECLARE_CLIENTCLASS();
 	DECLARE_DATADESC();
...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_DT(C_NPC_Drone, DT_NPC_Drone, CNPC_Drone)
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Drone::OnDataChanged( DataUpdateType_t type )
{
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Drone::UpdateOnRemove( void )
{
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Drone::SoundInit( void )
{
	if (( m_nEnginePitch1 < 0 ) || ( m_nEnginePitch2 < 0 ) )
		return;
	...
	if( !m_pEngineSound1 )
	{
		m_pEngineSound1 = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), "NPC_Drone.EngineSound1" );
	...
	if( !m_pEngineSound2 )
	{
		m_pEngineSound2 = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), "NPC_Drone.EngineSound2" );
	...
	if( !m_pBladeSound )
	{
		m_pBladeSound = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), "NPC_Drone.BladeSound" );
...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Drone::SoundShutdown(void)
{
	// Kill the engine!
	if ( m_pEngineSound1 )
	{
